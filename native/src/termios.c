...
int serial_test(char * filename) {
	unsigned long *hcomm;
	int ret;
	hcomm = CreateFile(filename, GENERIC_READ | GENERIC_WRITE, 0, 0,
			OPEN_EXISTING, 0, 0);
...
void termios_setflags(int fd, int termios_flags[]) {
	...
	int i, result;
	int windows_flags[11] = { 0, EV_RXCHAR, EV_TXEMPTY, EV_CTS, EV_DSR, EV_RING
			| 0x2000, EV_RLSD, EV_ERR, EV_ERR, EV_ERR, EV_BREAK };
	if (!index) {
...
const char *get_dos_port(char const *name) {
	ENTER( "get_dos_port" );
	if (!strcmp(name, "/dev/cua0"))
		return ("COM1");
	if (!strcmp(name, "/dev/cua1"))
		return ("COM2");
	if (!strcmp(name, "/dev/cua2"))
		return ("COM3");
	if (!strcmp(name, "/dev/cua3"))
		return ("COM4"); LEAVE( "get_dos_port" );
...
int serial_close(int fd) {
	...
	if (index) {
		if (index->rol.hEvent)
			CloseHandle(index->rol.hEvent);
		if (index->wol.hEvent)
			CloseHandle(index->wol.hEvent);
		if (index->sol.hEvent)
			CloseHandle(index->sol.hEvent);
		if (index->hComm)
			CloseHandle(index->hComm);
		if (index->ttyset)
			free(index->ttyset);
...
void cfmakeraw(struct termios *s_termios) {
	ENTER( "cfmakeraw" );
	s_termios->c_iflag &= ~( IGNBRK | BRKINT | PARMRK | ISTRIP | INLCR | IGNCR
			| ICRNL | IXON);
...
int open_port(struct termios_list *port) {
	ENTER( "open_port" );
	port->hComm = CreateFile(port->filename, GENERIC_READ | GENERIC_WRITE, 0, 0,
...
struct termios_list *find_port(int fd) {
	...
	fail: sprintf(message, "No info known about the port. %i\n", fd);
	report(message);
	set_errno( EBADF);
	LEAVE( "find_port" );
	return NULL;
...
struct termios_list *add_port(const char *filename) {
	...
	fail: report("add_port:  Out Of Memory\n");
	if (port->ttyset)
		free(port->ttyset);
	if (port->astruct)
		free(port->astruct);
...
int serial_write(int fd, char *Str, int length) {
	...
	if (!WriteFile(index->hComm, Str, length, &nBytes, &index->wol)) {
		...
		/* This is breaking on Win2K, WinXP for some reason */
			...
			while (!GetOverlappedResult(index->hComm, &index->wol, &nBytes,
					TRUE)) {
	...
	while (size > 0) {
		...
		if (!err) {
			...
			case ERROR_IO_PENDING:
				while (!GetOverlappedResult(index->hComm, &index->rol, &nBytes,
						TRUE)) {
					if (GetLastError() != ERROR_IO_INCOMPLETE) {
						ClearErrors(index, &stat);
						return (total);
...
int termios_to_DCB(struct termios *s_termios, DCB *dcb) {
	...
	if (s_termios->c_cflag & PARENB) {
		if (s_termios->c_cflag & PARODD && s_termios->c_cflag & CMSPAR) {
...
int tcsetattr(int fd, int when, struct termios *s_termios) {
	...
	if (!(s_termios->c_cflag & CIGNORE)) {
		...
		/* baudrate */
		if (termios_to_DCB(s_termios, &dcb) < 0)
			return -1;
	} else {
	}
...
int ioctl(int fd, int request, ...) {
	...
	case TIOCOUTQ:
		...
		return -ENOIOCTLCMD;
	default:
		sprintf(message, "FIXME:  ioctl: unknown request: %#x\n", request);
		report(message);
		va_end(ap);
		return -ENOIOCTLCMD;
	}
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
#ifndef __LCC__
int serial_select(int n, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
