...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
struct event_info_struct build_threadsafe_eis(JNIEnv *env, jobject *jobj,
	...
	myeis.send_event = (*env)->GetMethodID(env, myeis.jclazz, "sendEvent",
			"(IZ)Z");
	return (myeis);
}
...
jfieldID jfdataBits = (*env)->GetFieldID(env, jclazz, "dataBits", "I");
jfieldID jfstopBits = (*env)->GetFieldID(env, jclazz, "stopBits", "I");
jfieldID jfparity = (*env)->GetFieldID(env, jclazz, "parity", "I");
...
#endif
(*env)->SetIntField(env, jobj, jfspeed, (jint) get_java_baudrate(baudrate));
...
/*----------------------------------------------------------
 ...
 device file or bios has the device disabled.
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(open)(JNIEnv *env, jobject jobj, jstring jstr) {
int fd;
int pid = -1;
char message[80];
const char *filename;
...
fail: (*env)->ReleaseStringUTFChars(env, jstr, filename);
LEAVE( "RXTXPort:open" );
	...
	jobject jobj,jstring jstr )
{
	...
	*/
	...
	ENTER("RXTXPort:nativeClose");if (fd > 0)
	{
	...
	(*env)->ReleaseStringUTFChars( env, jstr, filename );
	LEAVE(
	"RXTXPort:nativeClose"); report_time_end( );
return;
}
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
int set_port_params(JNIEnv *env, int fd, int cspeed, int dataBits, int stopBits,
int parity) {
struct termios ttyset;
int result = 0;
...
cspeed = 38400;
#endif /* TIOCGSERIAL */
if (cfsetispeed(&ttyset, cspeed) < 0 || cfsetospeed(&ttyset, cspeed) < 0) {
/*
...
#endif /* TIOCGSERIAL */
if (cfsetispeed(&ttyset, B38400) < 0 || cfsetospeed(&ttyset, B38400) < 0) {
...
/*----------------------------------------------------------
 ...
 exceptions: UnsupportedCommOperationException
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(nativeSetSerialPortParams)(JNIEnv *env,
jobject jobj, jint speed, jint dataBits, jint stopBits, jint parity) {
int fd = get_java_var(env, jobj, "fd", "I");
...
JNIEXPORT jint JNICALL RXTXPort(nativeGetFlowControlMode)(JNIEnv *env,
jobject jobj, jint fd) {
struct termios ttyset;
int ret = 0;
...
JNIEXPORT jint JNICALL RXTXPort(nativeGetParity)(JNIEnv *env, jobject jobj,
jint fd) {
struct termios ttyset;
...
#ifdef  CMSPAR
if ((ttyset.c_cflag & PARENB) && (ttyset.c_cflag & PARODD)
	&& (ttyset.c_cflag & CMSPAR)) {
return ( JPARITY_MARK);
} else if (ttyset.c_cflag & ( PARENB && ttyset.c_cflag & CMSPAR)) {
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(nativeDrain)(JNIEnv *env, jobject jobj,
jboolean interrupted) {
int fd = get_java_var(env, jobj, "fd", "I");
...
if (result)
throw_java_exception(env, IO_EXCEPTION, "nativeDrain", strerror( errno));
if (interrupted)
return (JNI_FALSE);
#if !defined(TIOCSERGETLSR) && !defined(WIN32)
if( eis && eis->writing )
	...
	jobject jobj, jint duration )
{
	int fd = get_java_var( env, jobj,"fd","I" );
	report_time_start( );
	ENTER("RXTXPort:sendBreak()");tcsendbreak( fd, (int)( duration / 250 ) );
	report_time_end( );
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(NativegetReceiveTimeout)(JNIEnv *env,
jobject jobj) {
int fd = get_java_var(env, jobj, "fd", "I");
struct termios ttyset;
...
if (tcgetattr(fd, &ttyset) < 0)
goto fail; LEAVE( "RXTXPort:nativegetRecieveTimeout()" );
return (ttyset.c_cc[ VTIME] * 100);
fail: LEAVE( "RXTXPort:nativegetRecieveTimeout()" );
...
/*----------------------------------------------------------
 ...
 comments:   see  NativeEnableReceiveTimeoutThreshold
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(NativeisReceiveTimeoutEnabled)(JNIEnv *env,
jobject jobj) {
int fd = get_java_var(env, jobj, "fd", "I");
struct termios ttyset;
ENTER( "RXTXPort:NativeisRecieveTimeoutEnabled()" );
if (tcgetattr(fd, &ttyset) < 0)
goto fail; LEAVE( "RXTXPort:NativeisRecieveTimeoutEnabled()" );
return (ttyset.c_cc[ VTIME] > 0 ? JNI_TRUE : JNI_FALSE);
fail: LEAVE( "RXTXPort:NativeisRecieveTimeoutEnabled()" );
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(isDSR)(JNIEnv *env, jobject jobj) {
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(isCD)(JNIEnv *env, jobject jobj) {
unsigned int result = 0;
int fd = get_java_var(env, jobj, "fd", "I");
char message[80];
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(isCTS)(JNIEnv *env, jobject jobj) {
unsigned int result = 0;
int fd = get_java_var(env, jobj, "fd", "I");
char message[80];
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(isRI)(JNIEnv *env, jobject jobj) {
unsigned int result = 0;
int fd = get_java_var(env, jobj, "fd", "I");
char message[80];
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(isRTS)(JNIEnv *env, jobject jobj) {
unsigned int result = 0;
int fd = get_java_var(env, jobj, "fd", "I");
char message[80];
	...
	jobject jobj, jboolean state )
{
	...
	ENTER("RXTXPort:setRTS");ioctl( fd, TIOCMGET, &result );
	...
	jobject jobj, jboolean state )
{
	...
	ENTER("RXTXPort:setDSR()");ioctl( fd, TIOCMGET, &result );
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(isDTR)(JNIEnv *env, jobject jobj) {
unsigned int result = 0;
int fd = get_java_var(env, jobj, "fd", "I");
char message[80];
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(nativeSetBaudBase)(JNIEnv *env,
...
if (sstruct.baud_base < 1 || ioctl(fd, TIOCSSERIAL, &sstruct) < 0) {
goto fail;
}
return ((jboolean) 0);
fail: throw_java_exception(env, IO_EXCEPTION, "nativeSetBaudBase",
strerror( errno));
return ((jboolean) 1);
#else
throw_java_exception( env, UNSUPPORTED_COMM_OPERATION,
...
/*----------------------------------------------------------
 ...
 First introduced in rxtx-2.1-3
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(nativeGetBaudBase)(JNIEnv *env, jobject jobj) {
...
fail: throw_java_exception(env, IO_EXCEPTION, "nativeGetBaudBase",
strerror( errno));
return ((jint) - 1);
#else
throw_java_exception( env, UNSUPPORTED_COMM_OPERATION,
...
/*----------------------------------------------------------
 ...
 First introduced in rxtx-2.1-3
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(nativeSetDivisor)(JNIEnv *env, jobject jobj,
jint Divisor) {
...
if (sstruct.custom_divisor < 1 || ioctl(fd, TIOCSSERIAL, &sstruct) < 0) {
goto fail;
}
return ((jboolean) 0);
fail: throw_java_exception(env, IO_EXCEPTION, "nativeSetDivisor",
strerror( errno));
return ((jboolean) 1);
#else
throw_java_exception( env, UNSUPPORTED_COMM_OPERATION,
...
/*----------------------------------------------------------
 ...
 First introduced in rxtx-2.1-3
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(nativeGetDivisor)(JNIEnv *env, jobject jobj) {
...
fail: throw_java_exception(env, IO_EXCEPTION, "nativeGetDivisor",
strerror( errno));
return ((jint) - 1);
#else
throw_java_exception( env, UNSUPPORTED_COMM_OPERATION,
...
pid = getpid();
	...
	{
		(*env)->ReleaseStringUTFChars( env, jstr, filename );
		LEAVE("RXTXPort:nativeStaticSetSerialPortParams");throw_java_exception( env, UNSUPPORTED_COMM_OPERATION,
	...
	if( set_port_params( env, fd, cspeed, dataBits, stopBits, parity ) )
	{
		(*env)->ReleaseStringUTFChars( env, jstr, filename );
		LEAVE(
"RXTXPort:nativeStatic SetSerialPortParams");throw_java_exception( env, UNSUPPORTED_COMM_OPERATION,
			"nativeStaticSetSerialPortParams", strerror( errno ) );
		return;
	}
	...
	(*env)->ReleaseStringUTFChars( env, jstr, filename );
	LEAVE(
"RXTXPort:nativeStaticSetSerialPortParams");return;
fail:
	(*env)->ReleaseStringUTFChars( env, jstr, filename );
	LEAVE(
"RXTXPort:nativeStaticSetSerialPortParams");
...
/*----------------------------------------------------------
 ...
 comments:    CD stands for carrier detect
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(nativeStaticIsCD)(JNIEnv *env, jobject jobj,
jstring jstr) {
unsigned int result = 0;
const char *filename = (*env)->GetStringUTFChars(env, jstr, 0);
int fd = find_preopened_ports(filename);
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(nativeStaticIsRI)(JNIEnv *env, jobject jobj,
jstring jstr) {
unsigned int result = 0;
const char *filename = (*env)->GetStringUTFChars(env, jstr, 0);
int fd = find_preopened_ports(filename);
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(nativeStaticGetBaudRate)(JNIEnv *env,
jobject jobj, jstring jstr) {
const char *filename = (*env)->GetStringUTFChars(env, jstr, 0);
int fd = find_preopened_ports(filename);
struct termios ttyset;
...
/*----------------------------------------------------------
 ...
 comments:    simple test for preopened ports
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(nativeStaticGetDataBits)(JNIEnv *env,
jobject jobj, jstring jstr) {
const char *filename = (*env)->GetStringUTFChars(env, jstr, 0);
int fd = find_preopened_ports(filename);
struct termios ttyset;
...
/*----------------------------------------------------------
 ...
 comments:    simple test for preopened ports
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(nativeStaticGetParity)(JNIEnv *env,
jobject jobj, jstring jstr) {
const char *filename = (*env)->GetStringUTFChars(env, jstr, 0);
int fd = find_preopened_ports(filename);
struct termios ttyset;
...
/*----------------------------------------------------------
 ...
 comments:    simple test for preopened ports
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(nativeStaticGetStopBits)(JNIEnv *env,
jobject jobj, jstring jstr) {
const char *filename = (*env)->GetStringUTFChars(env, jstr, 0);
int fd = find_preopened_ports(filename);
struct termios ttyset;
...
if (tcgetattr(fd, &ttyset) < 0)
goto fail; LEAVE( "nativeGetEndOfInputChar" );
return ((jbyte) ttyset.c_cc[VEOF]);
fail: LEAVE( "nativeGetEndOfInputChar" );
...
throw_java_exception(env, UNSUPPORTED_COMM_OPERATION, "Not implemented... yet",
strerror( errno));
LEAVE( "nativeSetParityErrorChar" );
return (JNI_FALSE);
#endif /* WIN32 */
...
if (tcsetattr(fd, TCSANOW, &ttyset) < 0)
goto fail; LEAVE( "nativeSetEndOfInputChar" );
return (JNI_TRUE);
fail: throw_java_exception(env, IO_EXCEPTION, "nativeSetEndOfInputChar",
strerror( errno));
	...
	JNIEnv *env, jobject jobj, jint vtime, jint threshold, jint buffer)
{
	...
	ENTER(
"RXTXPort:NativeEnableRecieveTimeoutThreshold");if( tcgetattr( fd, &ttyset ) < 0 ) goto fail;
	ttyset.c_cc[ VMIN ] = threshold;
	ttyset.c_cc[ VTIME ] = timeout/100;
	if( tcsetattr( fd, TCSANOW, &ttyset ) < 0 ) goto fail;

	LEAVE(
"RXTXPort:NativeEnableRecieveTimeoutThreshold");return;
fail:
	LEAVE(
"RXTXPort:NativeEnableRecieveTimeoutThreshold");
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(readArray)(JNIEnv *env, jobject jobj,
jbyteArray jbarray, jint offset, jint length) {
int bytes;
jbyte *body;
/* char msg[80]; */
int fd = get_java_var(env, jobj, "fd", "I");
...
throw_java_exception(env, ARRAY_INDEX_OUT_OF_BOUNDS, "readArray",
"Invalid length");
return -1;
}
body = (*env)->GetByteArrayElements(env, jbarray, 0);
bytes = read_byte_array(env, &jobj, fd, (unsigned char *) (body + offset),
length, timeout);/* dima */
(*env)->ReleaseByteArrayElements(env, jbarray, body, 0);
if (bytes < 0) {
report("RXTXPort:readArray bytes < 0");
LEAVE( "RXTXPort:readArray" );
throw_java_exception(env, IO_EXCEPTION, "readArray", strerror( errno));
return -1;
}
/*
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(readTerminatedArray)(JNIEnv *env, jobject jobj,
jbyteArray jbarray, jint offset, jint length, jbyteArray jterminator) {
int bytes, total = 0;
...
do {
bytes = read_byte_array(env, &jobj, fd,
(unsigned char *) (body + offset + total), 1, timeout);/* dima */
total += bytes;
if (bytes < 0) {
report("RXTXPort:readArray bytes < 0");
LEAVE( "RXTXPort:readArray" );
...
/*----------------------------------------------------------
 ...
 exceptions:  none
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(nativeavailable)(JNIEnv *env, jobject jobj) {
int fd = get_java_var(env, jobj, "fd", "I");
int result;
/*
 char message[80];
 ...
 */
return (jint) result;
fail: report("RXTXPort:nativeavailable:  ioctl() failed\n");
/*
 LEAVE( "RXTXPort:nativeavailable" );
 */
	...
	jobject jobj, jint flowmode )
{
	...
	ENTER("RXTXPort:setflowcontrol");if( tcgetattr( fd, &ttyset ) ) goto fail;
...
/* TRENT */
	...
	LEAVE(
"RXTXPort:setflowcontrol");return;
fail:
	LEAVE("RXTXPort:setflowcontrol"); throw_java_exception( env, UNSUPPORTED_COMM_OPERATION, "",
...
report_verbose(
" driver_has_tiocgicount:  Port does not support TIOCGICOUNT events\n");
return (0);
} else
return (1);
#endif /*  TIOCGICOUNT */
...
JNIEXPORT jboolean JNICALL RXTXCommDriver(testRead)(JNIEnv *env, jobject jobj,
jstring tty_name, jint port_type) {
struct termios ttyset;
char c;
...
return (*env)->NewStringUTF(env, DEVICEDIR); LEAVE( "RXTXCommDriver:getDeviceDirectory" );
}
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort(setInputBufferSize)
(JNIEnv *env,
...
/*----------------------------------------------------------
 ...
 comments:    see fopen/fclose/fwrite/fread man pages.
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(getInputBufferSize)(JNIEnv *env, jobject jobj) {
...
/*----------------------------------------------------------
 ...
 comments:    see fopen/fclose/fwrite/fread man pages.
 ----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort(setOutputBufferSize)
(JNIEnv *env,
jobject jobj, jint size )
{
...
/*----------------------------------------------------------
 ...
 comments:    see fopen/fclose/fwrite/fread man pages.
 ----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort(getOutputBufferSize)(JNIEnv *env, jobject jobj) {
...
(*env)->ExceptionClear(env);
result = (*env)->CallBooleanMethod(env, *eis->jobj, eis->checkMonitorThread);
#ifdef DEBUG
...
if (fd < 0) {
snprintf(message, 79, "RXTX fhs_lock() Error: creating lock file: %s: %s\n",
file, strerror(errno));
report_error(message);
return 1;
}
...
LOCKDIR, (int) major(buf.st_dev), (int) major(buf.st_rdev),
...
if (fd < 0) {
sprintf(message, "RXTX uucp_lock() Error: creating lock file: %s\n",
lockfilename);
report_error(message);
return 1;
}
...
sprintf(file, LOCKDIR"/LK.%03d.%03d.%03d", (int) major(buf.st_dev),
...
testLockAbsFileName = calloc(
strlen(testLockFileDirName) + strlen(testLockFileName) + 2, sizeof(char));
...
report_error(
"check_group_uucp(): mktemp malformed string - \
			should not happen");
...
"/usr/spool/uucp/LCK", "/var/lock", "/var/lock/modem", "/var/spool/lock",
"/var/spool/locks", "/var/spool/uucp",
LOCKDIR, NULL };
 ...
 */
if (!stat(lockdirs[i], &buf2) && buf2.st_ino != lockbuf.st_ino
...
/* FHS style */
asprintf(&file, "%s/%s%s", lockdirs[i], lockprefixes[k], p);
if (stat(file, &buf) == 0) {
asprintf(&message, UNEXPECTED_LOCK_FILE, file);
...
asprintf(&file, "%s/%s%03d.%03d.%03d", lockdirs[i], lockprefixes[k],
(int) major(buf.st_dev), (int) major(buf.st_rdev), (int) minor(buf.st_rdev));
if (stat(file, &buf) == 0) {
asprintf(&message, UNEXPECTED_LOCK_FILE, file);
report_warning(message);
free(message);
