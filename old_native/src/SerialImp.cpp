...
/*----------------------------------------------------------
 ...
 exceptions:  none
 ----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort(nativeClose)
( JNIEnv *env,
	jobject jobj )
{
...
data_rate translate_speed(JNIEnv *env, jint speed) {
	...
	case 230400:
		return B_230400_BPS;
	case 31250:
		return B_31250_BPS;  // this one was in BeOS's SerialPort.h, but I am
...
/*----------------------------------------------------------
 ...
 exceptions:  IOException
 ----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort(writeByte)
( JNIEnv *env,
...
RXTXPort.writeArray
   ...
   exceptions:  IOException
----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort(writeArray)
( JNIEnv *env,
...
----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort(drain)
( JNIEnv *env,
	jobject jobj )
{
...
----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort(sendBreak)
( JNIEnv *env,
...
/*----------------------------------------------------------
 ...
 ----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort(setRTS)
( JNIEnv *env,
...
RXTXPort.setDSR
   ...
   comments:    tcsetattr with c_cflag CRTS_IFLOW
----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort(setDSR)
( JNIEnv *env,
		jobject jobj, jboolean state )
{
...
/*----------------------------------------------------------
 ...
 flow control
 ----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort(setflowcontrol)
( JNIEnv *env,
...
for (i = 0; i < 64; i++) {
	...
	if (S_ISCHR(mystat.st_mode)) {
		printf("inside RXTXCommDriver:isDeviceGood() trying to open %s.  ",
				teststring);
		fd = open(teststring, O_RDONLY | O_NONBLOCK);
		printf("got %i\n", fd);
		if (fd > 0) {
			printf(
					"inside RXTXCommDriver:isDeviceGood() %s is valid on this system.\n",
					teststring);
			close(fd);
			result = JNI_TRUE;
			break;
